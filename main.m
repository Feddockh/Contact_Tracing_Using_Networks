...
users = data_generator(dims, num_people, timesteps);
...
for t = 1:timesteps
    ...
    for userID = 1:length(users)
        ...
        while (isempty(pCoordinates1) && apID <= length(access_points))
            ...
            pCoordinates1 = coordinateCalc(users(userID), access_points(apID), dims, t);
        ...
        for i = apID:length(access_points)
            ...
            pCoordinates2 = coordinateCalc(users(userID), access_points(i), dims, t);
...
for i = 1:length(contacts)
    ...
    for j = 1:length(contactedUsers)
        if (trace_userID == contactedUsers(j).getID())
            ...
            % Retrieve the centrainties that the user is at that location
            ...
            for k = 1:length(contactedUsers)
                ...
                if (j ~= k)
                    ...
                    % Add the certainty of this contact to the overall
                    % certainty
